    ...
    <NicknameGate>
      ...
      <main className="max-w-xl mx-auto px-4 py-6 flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <h1 className="display text-xl" style={{ color: 'var(--ink)' }}>✉ messages</h1>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16" style={{ color: 'var(--text2)' }}>
            ...
            <p className="text-sm font-bold" style={{ color: 'var(--ink)' }}>no dms yet.</p>
            <p className="text-xs mt-2">tap <strong>✉ dm</strong> on someone&apos;s post to start a chat.</p>
          </div>
        ) : (
          convs.map(conv => {
            ...
            return (
              <Link key={conv.id} href={`/dm/${conv.id}`} className="card card-tap p-4 flex items-center gap-3 cursor-pointer">
                <div
                  className="w-10 h-10 rounded-lg flex items-center justify-center text-sm font-bold shrink-0 mono"
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs ml-2 shrink-0 mono" style={{ color: 'var(--text2)' }}>
